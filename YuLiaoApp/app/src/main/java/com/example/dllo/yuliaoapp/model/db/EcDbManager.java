 ...
 */
public class EcDbManager {
    static private EcDbManager dbMgr = new EcDbManager();
    private DbOpenHelper dbHelper;

    private EcDbManager(){
        dbHelper = DbOpenHelper.getInstance(C_MyApp.getInstance().getApplicationContext());
    }

    public static synchronized EcDbManager getInstance(){
        if(dbMgr == null){
            dbMgr = new EcDbManager();
        }
        return dbMgr;
    }
    ...
    synchronized public Map<String, EaseUser> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (username.equals(C_Constant.NEW_FRIENDS_USERNAME) || username.equals(C_Constant.GROUP_USERNAME)
                        || username.equals(C_Constant.CHAT_ROOM)|| username.equals(C_Constant.CHAT_ROBOT)) {
